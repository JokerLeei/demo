...
import com.example.demo.excel.write.converter.LongestMatchColumnWidthData;
import com.example.demo.excel.write.data.ComplexHeadData;
import com.example.demo.excel.write.data.ConverterData;
import com.example.demo.excel.write.data.DemoData;
import com.example.demo.excel.write.data.DemoMergeData;
import com.example.demo.excel.write.data.DemoStyleData;
import com.example.demo.excel.write.data.ImageData;
import com.example.demo.excel.write.data.IndexData;
import com.example.demo.excel.write.data.WidthAndHeightData;
import com.example.demo.excel.write.handler.CommentWriteHandler;
import com.example.demo.excel.write.handler.CustomCellWriteHandler;
import com.example.demo.excel.write.handler.CustomSheetWriteHandler;
 ...
 *
 * @author: lijiawei04
 * @date: 2021/2/25 8:07 下午
 */
@Ignore
public class WriteTest {

    private static final String BASE_PATH = "/Users/bjhl/IdeaProjects/demo/excel/";
    ...
    public void simpleWrite() {
        // ----- 写法1 -----
        String fileName = BASE_PATH + "simpleWrite_1_" + System.currentTimeMillis() + ".xlsx";
        ...
        // ----- 写法2 -----
        fileName = BASE_PATH + "simpleWrite_2_" + System.currentTimeMillis() + ".xlsx";
        // 这里 需要指定写用哪个class去写
    ...
    public void excludeOrIncludeWrite() {
        String fileName = BASE_PATH + "excludeOrIncludeWrite_1_" + System.currentTimeMillis() + ".xlsx";

        // 根据用户传入字段 假设我们要忽略 date
        // 这里 需要指定写用哪个class去写，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
        EasyExcel.write(fileName, DemoData.class)
                .excludeColumnFiledNames(Sets.newHashSet("date"))
                .sheet("模板")
                .doWrite(data());


        fileName = BASE_PATH + "excludeOrIncludeWrite_2_" + System.currentTimeMillis() + ".xlsx";
        // 根据用户传入字段 假设我们只要导出 date
        // 这里 需要指定写用哪个class去写，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
        EasyExcel.write(fileName, DemoData.class)
                .includeColumnFiledNames(Sets.newHashSet("date"))
                .sheet("模板")
                .doWrite(data());
    }
    ...
    public void repeatedWrite() {
        // 方法1 如果写到同一个sheet
        String fileName = BASE_PATH + "repeatedWrite_1_" + System.currentTimeMillis() + ".xlsx";
        ExcelWriter excelWriter = null;
        try {
            // 这里 需要指定写用哪个class去写
            excelWriter = EasyExcel.write(fileName, DemoData.class).build();
        ...
        fileName = BASE_PATH + "repeatedWrite_2_" + System.currentTimeMillis() + ".xlsx";
        try {
            // 这里 指定文件
            excelWriter = EasyExcel.write(fileName, DemoData.class).build();
        ...
        fileName = BASE_PATH + "repeatedWrite_3_" + System.currentTimeMillis() + ".xlsx";
        try {
            // 这里 指定文件
    ...
    public void imageWrite() throws Exception {
        ...
        try {
            ...
            list.add(imageData);
            // 放入五种类型的图片(File、InputStream、String、byte[]、URL) 实际使用只要选一种即可
            // 1. byte[]
            String imagePath = BASE_PATH + "img" + File.separator + "img.jpg";
